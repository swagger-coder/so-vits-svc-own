...
def process_one(hps, filename, hmodel,f0p, sampling_rate, diff=False,mel_extractor=None):
    # print(filename)
...
def process_batch(hps, file_chunk, f0p, speech_encoder, sampling_rate, diff=False, mel_extractor=None):
    ...
    for filename in tqdm(file_chunk):
        process_one(hps, filename, hmodel, f0p, sampling_rate, diff, mel_extractor)

def parallel_process(hps, filenames, num_processes, f0p, diff, mel_extractor, speech_encoder, sampling_rate):
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        ...
        for i in range(num_processes):
            ...
            tasks.append(executor.submit(process_batch, hps, file_chunk, f0p, speech_encoder, sampling_rate, diff, mel_extractor))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--num_processes', type=int, default=8, help='You are advised to set the number of processes to the same as the number of CPU cores'
    )
    args = parser.parse_args()
    name = args.in_dir.split("/")[-1]
    hps = utils.get_hparams_from_file(f"configs/config_{name}.json")
    # dconfig = du.load_config(f"configs/diffusion_{name}.yaml")
    ...
    parallel_process(hps, filenames, num_processes, f0p, args.use_diff, mel_extractor, speech_encoder, sampling_rate)