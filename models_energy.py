...
class SynthesizerTrn(nn.Module):
    ...
    def __init__(self,
        ...
        self.mel_prenet = nn.Conv1d(80, hidden_channels, 3, padding=1)
    ...
    def forward(self, c, f0, uv, spec, g=None, c_lengths=None, spec_lengths=None, vol = None):
        ...
        # mel energy
        decoder_input = x
        # print("decoder_input", decoder_input.shape, "lf0", lf0.shape, "self.f0_prenet(lf0)", self.f0_prenet(lf0).shape,"spec_lengths", spec_lengths, "c_lengths", c_lengths, "decoder_input + self.f0_prenet(lf0)", (decoder_input + self.f0_prenet(lf0)).shape)
        predict_mel, predict_bn_mask = self.mel_decoder(decoder_input + self.f0_prenet(lf0), spec_lengths, spk_emb=g)

        predict_energy = predict_mel.detach().sum(1).unsqueeze(1) / 80 # 80 is mel channels
        ...
        # encoder
        z_ptemp, m_p, logs_p, _ = self.enc_p(decoder_input, x_mask, f0=f0_to_coarse(f0))
        ...
        return o, ids_slice, spec_mask, (z, z_p, m_p, logs_p, m_q, logs_q), pred_lf0, norm_lf0, lf0, predict_mel
    ...
    def infer(self, c, f0, uv, g=None, noice_scale=0.35, seed=52468, predict_f0=False, vol = None):
        ...
        z_p, m_p, logs_p, c_mask = self.enc_p(decoder_input, x_mask, f0=f0_to_coarse(f0), noice_scale=noice_scale)
