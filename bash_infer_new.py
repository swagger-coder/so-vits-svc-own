...
def process_one(model, speaker, wav):
    cmd = "CUDA_VISIBLE_DEVICES=3 python inference_main.py -m {3} \
        -c logs/{0}/config.json -n {2} -t 0 -s {1} -wf wav -i {4} -f0p dio -a".format(model, speaker, wav, utils.latest_checkpoint_path(os.path.join("logs", model), "G_*.pth"), input_path)
    os.system(cmd)
    print(cmd)
...
def main():
    ...
    parser.add_argument('-n', '--in_dir', type=str, nargs='+', default=["君の知らない物語-src.wav"], help='wav文件名列表，放在raw文件夹下')
    parser.add_argument('-t', '--trans', type=int, nargs='+', default=[0], help='音高调整，支持正负（半音）')
    parser.add_argument('-s', '--spk_list', type=str, nargs='+', default=['buyizi'], help='合成目标说话人名称')
    parser.add_argument('-i', '--input', type=str, default="zh", help='infer')
    ...
    # input_path = os.path.join("raw", args.input)
    ...
    clean_names = os.listdir(in_dir)
    infer_tool.fill_a_to_b(trans, clean_names)
    for clean_name, tran in zip(clean_names, trans):
        raw_audio_path = os.path.join(in_dir, clean_name)
        ...
        for spk in spk_list:
            ...
            out_path = os.path.join("results", name, args.input)
            ...
            if use_spk_mix:
                spk = "spk_mix"
            res_path = os.path.join(out_path, f'{name}_{clean_name.split(".")[0]}_{key}_{spk}{cluster_name}_{isdiffusion}_{f0p}2.{wav_format}')
