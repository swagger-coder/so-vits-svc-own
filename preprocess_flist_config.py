...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_name", type=str, default="caicai", help="path to filelist dir")
    ...
    filelist_dir = os.path.join("./filelists", args.data_name)
    os.makedirs(filelist_dir, exist_ok=True)

    train_list = os.path.join(filelist_dir, "train.txt")
    val_list = os.path.join(filelist_dir, "val.txt")
    ...
    # d_config_template["env"]["expdir"] = f"logs/{args.data_name}/diffusion"
    
    config_template["spk"] = spk_dict
    config_template["model"]["n_speakers"] = 210
    ...
    if args.speech_encoder == "vec768l12" or args.speech_encoder == "dphubert" or args.speech_encoder == "wavlmbase+":
        ...
        config_template["model"]["filter_channels"] = 768
    ...
    print(f"Writing config_{args.data_name}.json")
    with open(f"configs/config_{args.data_name}.json", "w", encoding="utf-8") as f:
        json.dump(config_template, f, indent=2, ensure_ascii=False)
    # print(f"Writing configs/diffusion_{args.data_name}.yaml")
    # du.save_config(f"configs/diffusion_{args.data_name}.yaml",d_config_template)
